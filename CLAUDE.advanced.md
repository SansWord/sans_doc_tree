<!-- THE FULL / ADVANCED VERSION — a menu, not a file to load wholesale. Your auto-loaded root is the
     lean CLAUDE.md (only CLAUDE.md / AGENTS.md is auto-loaded). As your project grows and you want more
     structure, copy the blocks you want from here into CLAUDE.md. Keep the root small. -->

> **Using this menu.** Copy a block below into your lean `CLAUDE.md` when you feel its absence — and
> **replace** the matching stub there rather than duplicating it (one fact, one place). If you take
> just one block, take **Locked decisions**; next, **Before you plan — consult the tree**. Defer
> Conventions / PR gate / Devlog format / Before committing until you actually want them.

# <!-- TODO: Project Name --> — Project Context

<!-- One short paragraph: what this project is. The root file is AUTO-LOADED into every coding session,
...
> (or an ADR under `docs/decisions/`). The superseded decision stays in the historical record, so the
> *why* of the change is preserved — then the new decision is the one to honor.
...
- **Maintained `docs/*.md` (authoritative, kept up to date):** the source of truth; must match the
  code. The agent may **trust these for quick answers without opening the source** — which is exactly
  why a stale maintained doc is dangerous: it doesn't just fail to help, it *actively misleads*. Keep
  them exact. When you add a `docs/*.md`, add it to this list with a one-line description **and its
  update trigger** (when it must be refreshed).
...
- **Historical (allowed to stale, but kept forever as decision history):** `docs/specs/*`,
  ...
  *(Optional historical types: `docs/decisions/` for ADRs — one record per significant decision; and
  `docs/meeting-minutes/` for human-discussion decisions/new directions.)*
  **ADRs are historical:** an ADR records *"we decided X, and why, on this date."* The **current rule**
  that decision enforces lives in a **maintained** doc (a `docs/*.md` or Locked decisions) — that's
  what the agent obeys. The ADR is the paper trail; the maintained doc is the source of truth.
...
**Review:** not yet
**References:** [spec](docs/specs/<file>.md) · [plan](docs/plans/<file>.md)   <!-- this version's spec/plan/ADR -->
**What was built:** <bullets>
**Key learnings:** <tagged bullets — [note] / [insight] / [gotcha]>
```

Linking each version to its spec/plan/ADR lets a later session pull the right history into context when
it revisits that module or traces a bug to a version. Keep a **TL;DR table** at the top — one row per
